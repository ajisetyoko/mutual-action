...
class IO():
    ...
    def load_model(self, model, **model_args):
        print('Before import_class')
        Model = import_class(model)
        print('Before import **model_args')
        model = Model(**model_args) # Profed that graph load in this line
        print(model)
        print('After import **model_args')
        self.model_text += '\n\n' + str(model)
        return model
